 ...
 */
class Magestore_ERPplus_Block_Adminhtml_Rewrite_Menu extends Mage_Adminhtml_Block_Page_Menu {
    ...
    protected function _buildMenuArray(Varien_Simplexml_Element $parent = null, $path = '', $level = 0) {
        ...
        foreach ($parent->children() as $childName => $child) {
            ...
            if (isset($childAttributes['module'])) {
                ...
                if ($this->helper('erpplus')->isERPmodule($moduleKey)) {
                    $erpmenu = true;
                }
            }
            if (!$erpmenu) {
                ...
                if ($childName == 'magestore_erpplus') {
                    if (count($this->_erpMenus)) {
                        foreach ($this->_erpMenus as $erpMenuKey => $erpMenu) {
                            $this->_erpMainMenu['magestore_erpplus']['children'][$erpMenuKey] = $erpMenu;
                        }
                    }
                }
            } else {
                if (!isset($this->_erpMenus[$moduleKey])) {                       
                    ...
                    if (isset($this->_erpMainMenu['magestore_erpplus'])) {
                        $this->_erpMainMenu['magestore_erpplus']['children'][$moduleKey] = $this->_erpMenus[$moduleKey];
                    }
                } else {
                    if ($child->children) {
                        $menuArr['children'] = $this->_buildMenuArray($child->children, $path . $childName . '/', $level + 1);
